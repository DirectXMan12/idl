...
type Flattener struct {
	...
	LookupReference func(ref string) (TypeIdent, error)
...
func (f *Flattener) FlattenType(typ TypeIdent) *apiext.JSONSchemaProps {
	...
	resSchema := f.FlattenSchema(*baseSchema)
...
func (f *Flattener) FlattenSchema(baseSchema apiext.JSONSchemaProps) *apiext.JSONSchemaProps {
...
// IdentFromRef splits a reference produced by the schema generator into its component
...
func IdentFromRef(ref string) (TypeIdent, error) {
	...
	return TypeIdent{
		GroupVersion: GroupVersion{Group: nameParts[0], Version: nameParts[1]},
		Name: nameParts[2],
	}, nil
}
...
func (f *flattenVisitor) Visit(baseSchema *apiext.JSONSchemaProps) SchemaVisitor {
	...
	if baseSchema.Ref != nil && len(*baseSchema.Ref) > 0 {
		// resolve this ref
		refIdent, err := f.LookupReference(*baseSchema.Ref)
		if err != nil {
			panic(err)
			return nil
		}
