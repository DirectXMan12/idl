...
var TopologyMarkers = []*definitionWithHelp{
	...
	must(markers.MakeDefinition("patchStrategy", markers.DescribesField, markers.RawArguments(nil))).
		WithHelp(StructType("").Help()),
	must(markers.MakeDefinition("patchMergeKey", markers.DescribesField, "")).
		WithHelp(StructType("").Help()),
}
